...
if ($method === "2") {
    ...
    if ($isSuccess == false) {
        ...
        $sql = "UPDATE PDXParameters SET Tag='2' WHERE AccountID=? AND Tag='1' AND DataType='sample'";
        ...
        // insert new job
        $jobid = uniqid("", TRUE);
        $sql = "INSERT INTO Jobs(JobID,Software,Analysis,Status,CreateTime) VALUES (?,\"pdx\",\"samplebatchupload\",0,now())";
        if ($result = $db->prepare($sql)) {
        ...
        $sql = "INSERT INTO PDXParameters(JobID,AccountID,Tag,DataType,XLSXFile) VALUES (?,?,1,\"sample\",?)";
        if ($result = $db->prepare($sql)) {
        ...
        echo json_encode(['goto' => 'samplelist.php?operate=view_print&uuid=allnew', 'error' => '']);
    }
}
$db->close();